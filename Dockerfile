...
ADD . /tesseract-opencv

WORKDIR /tesseract-opencv/build
...
RUN apt-get -qq update \
    ...
    && apt-get -qq install -y --no-install-recommends \
        ...
        libavformat-dev \
        libpq-dev \
    && pip install numpy pytesseract Pillow pandas jupyter \
    && wget -q https://github.com/opencv/opencv/archive/${OPENCV_VERSION}.zip -O opencv.zip \
    && unzip -qq opencv.zip -d /tesseract-opencv \
    && rm -rf opencv.zip \
    && cmake \
        -D BUILD_TIFF=ON \
        ...
        /tesseract-opencv/opencv-${OPENCV_VERSION} \
    && make -j$(nproc) \
    && make install \
    && rm -rf /tesseract-opencv/build/* \
    && rm -rf /tesseract-opencv/opencv-${OPENCV_VERSION} \
